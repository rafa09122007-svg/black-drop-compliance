      ...
      <div className="max-w-6xl mx-auto">
        ...
        <header className="flex justify-between items-end mb-8">
          <div>
            ...
            <p className="text-slate-500 font-medium text-sm">Equipment allocation and inspection tracking</p>
          </div>
          <button className="bg-slate-900 text-white px-6 py-2 rounded-xl font-bold text-sm hover:shadow-lg transition-all flex items-center gap-2">
            <Lucide.Box size={16} className="text-emerald-400" /> + Add Inventory
          </button>
        </header>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-10">
          <div className="bg-white p-6 rounded-2xl border-l-4 border-l-amber-500 shadow-sm flex items-center gap-4">
            <div className="p-3 bg-amber-50 text-amber-600 rounded-full">
              <Lucide.RefreshCw size={24} className="animate-spin" />
            </div>
            <div>
              <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Pending Inspections</p>
          ...
          <div className="bg-white p-6 rounded-2xl border-l-4 border-l-emerald-500 shadow-sm flex items-center gap-4">
            <div className="p-3 bg-emerald-50 text-emerald-600 rounded-full">
              <Lucide.ClipboardCheck size={24} />
            </div>
            <div>
              <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Fleet Readiness</p>
              ...
              {inventory.map((inv) => (
                <tr key={inv.id} className="hover:bg-slate-50 transition-colors">
                  <td className="px-8 py-5">
                    <div className="flex items-center gap-3">
                      <div className="p-2 bg-slate-100 rounded text-slate-400">
                        {inv.status === 'Warning' ? 
                          <Lucide.AlertOctagon size={16} className="text-amber-500" /> : 
                  ...
                  <td className="px-8 py-5 text-right">
                    <button className="text-slate-900 font-bold text-xs underline decoration-emerald-500 underline-offset-4">
                      Log Inspection
                    </button>
                  </td>
                </tr>
